...
if __name__ == "__main__":
    input_fold = Path(r"D:\code\HBMErrors\submit_fold\Fig15_diff_prediction_window")
    res_fold = input_fold.joinpath("result")
    res_fold.mkdir(exist_ok=True)

    down_sample = 20

    prediction_windows = ["1-hour"]#["1-hour", "1-day", "30-day"]

    for window in prediction_windows:
        window_input_fold = input_fold.joinpath(f"{window}_prediction_window")
        ...
        row_res_file = res_fold.joinpath(f"P{window}_row-level_predictor.csv")
        col_res_file = res_fold.joinpath(f"P{window}_col-level_predictor.csv")
        bank_res_file = res_fold.joinpath(f"P{window}_bank-level_predictor.csv")
        server_res_file = res_fold.joinpath(f"P{window}_server-level_predictor.csv")

        # Number of test
        for i in range(1):
            print(f"=======Test{i + 1} when using {window} prediction window=======\n")
